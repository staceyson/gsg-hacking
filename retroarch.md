...
To avoid installing build-time dependencies your computer manually, you can build the RetroArch in a Docker container:
    ...
    $ docker build --tag gsg-hacking . && docker run -it --mount "type=bind,source=$PWD,destination=$PWD" gsg-hacking:latest "${PWD}/build-sysroot.sh" libraries retroarch

## Installing RetroArch

Copy the `build/usr/arm-linux-gnueabihf/usr/bin/retroarch` executable to your Atari GSG using either ssh or sdcard.
